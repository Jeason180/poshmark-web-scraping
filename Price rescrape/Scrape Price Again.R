...
start_time <- Sys.time()
i <- 0
results <- mapply(ScrapePriceSize, missing_price$item_url[1:num], missing_price$item_id[1:num], SIMPLIFY = FALSE) %>% 
  bind_rows()
end_time <- Sys.time()
...
save(price_results, file = paste0("./price rescrape files/prices_", date, ".RDa"))
